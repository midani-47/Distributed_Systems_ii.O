...
## Bash Syntax
Same commands like the aforementioned, just Bash syntax for macOS/linux
### 1. Login as an admin user
...
```bash
curl -X 'GET' \
  'http://localhost:8080/api/auth/verify?token=YOUR_TOKEN'
```
...
```bash
curl -X 'POST' \
  'http://0.0.0.0:8081/api/transactions' \
...
```bash
curl -X 'POST' \
  'http://0.0.0.0:8081/api/transactions/1/results' \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -d '{
  "is_fraudulent": false,
...
### 4. Get transaction details including fraud prediction with the ID number after the "transactions/"

```bash 
curl -X 'GET' \
  'http://0.0.0.0:8081/api/transactions/1' \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
```bash
curl -X 'PUT' \
  'http://localhost:8081/api/transactions/1?status=accepted' \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
```bash
curl -X 'GET' \
  'http://localhost:8081/api/transactions/1/results' \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
